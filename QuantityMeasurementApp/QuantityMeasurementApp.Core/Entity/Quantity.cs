...
namespace QuantityMeasurementApp.Core.Entity
{
    ...
    public sealed class Quantity<U> where U : struct, Enum
    {
        ...
        //UC-12 Added a subtracting method with a specific target unit and with a source unit
        ...
        {
            if (other is null) throw new ArgumentException("Second operand cannot be null.", nameof(other));
            return SubtractUnit(this, other, targetUnit);
        }
        public Quantity<U> SubtractUnitTo(Quantity<U>other)
        {
            if (other is null) throw new ArgumentException("Second operand cannot be null.", nameof(other));
            return SubtractUnit(this, other, this.Unit);
        }

        //UC-12 For Divide method
        public static double DivideUnit(Quantity<U> q1, Quantity<U> q2, U targetUnit)
        {
            if (q1 is null) throw new ArgumentException("First operand cannot be null.", nameof(q1));
            if (q2 is null) throw new ArgumentException("Second operand cannot be null.", nameof(q2));
            ...
            if (double.IsNaN(q1.value) || double.IsInfinity(q1.value))
                throw new ArgumentException("Value must be a finite number.", nameof(q1.value));

            if (double.IsNaN(q2.value) || double.IsInfinity(q2.value))
                throw new ArgumentException("Value must be a finite number.", nameof(q2.value));

            var unit1Adapter = AdapterFor(q1.unit);
            var unit2Adapter = AdapterFor(q2.unit);
            var targetAdapter = AdapterFor(targetUnit);
            double bBase = unit1Adapter.ConvertToBaseUnit(q1.value);
            double btarget = unit2Adapter.ConvertToBaseUnit(q2.value);
            if (bBase == 0.0 ||btarget==0.0) throw new DivideByZeroException("Cannot divide by zero quantity.");
            double baseSum = bBase /btarget;
            

            return baseSum;

        }
        public double DivideUnitTo(Quantity<U> other, U targetUnit)
        {
            if (other is null) throw new ArgumentException("Second operand cannot be null.", nameof(other));
            return DivideUnit(this, other, targetUnit);
        }
        public double DivideUnitTo(Quantity<U> other)
        {
            if (other is null) throw new ArgumentException("Second operand cannot be null.", nameof(other));
            return DivideUnit(this, other, this.Unit);
        }
